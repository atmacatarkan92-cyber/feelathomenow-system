"""
Profit calculation: revenue (from tenancies) minus costs (from unit_costs) per unit/month.
"""
...
def calculate_unit_profit(session, unit_id: str, year: int, month: int) -> Dict[str, Any]:
    """
    For the given unit and month, compute revenue (from tenancies), costs (from unit_costs),
    and profit = revenue - costs.
    unit_costs are summed as monthly costs for that unit (no period filter).
    """
    ...
    costs = sum(float(c.amount_chf) for c in cost_rows)
