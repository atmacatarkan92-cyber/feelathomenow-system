"""
Admin landlord documents: upload to R2, list metadata (org-scoped).
Mirrors routes_admin_tenant_documents for Verwaltungen (landlords).
"""
...
from db.models import Landlord, LandlordDocument, User
from app.core.rate_limit import limiter
from app.core.r2_storage import (
    build_landlord_object_key,
    delete_object,
...
router = APIRouter(prefix="/api/admin", tags=["admin-landlord-documents"])
...
def _doc_to_dict(d: LandlordDocument, uploaded_by_name: Optional[str] = None) -> dict[str, Any]:
    return {
        "id": str(d.id),
        "organization_id": str(d.organization_id),
        "landlord_id": str(d.landlord_id),
...
def _resolve_object_key(doc: LandlordDocument) -> Optional[str]:
...
def _get_document_for_org(session: Session, document_id: str, org_id: str) -> LandlordDocument:
    doc = session.get(LandlordDocument, document_id)
    if not doc or str(getattr(doc, "organization_id", "")) != org_id:
        raise HTTPException(status_code=404, detail="Document not found")
    return doc


@router.get("/landlord-documents")
def admin_list_landlord_documents(
    ...
    rows: List[LandlordDocument] = list(
        session.exec(
            select(LandlordDocument)
            .where(LandlordDocument.landlord_id == landlord_id)
            .order_by(LandlordDocument.created_at.desc())
        ).all()
    )
    ...
    def name_for(doc: LandlordDocument) -> str:
...
@router.get("/landlord-documents/{document_id}/download")
def admin_landlord_document_download(
    document_id: str,
    org_id: str = Depends(get_current_organization),
    _=Depends(require_roles("admin", "manager")),
    session: Session = Depends(get_db_session),
...
@router.delete("/landlord-documents/{document_id}")
def admin_delete_landlord_document(
    document_id: str,
    org_id: str = Depends(get_current_organization),
...
@router.post("/landlord-documents")
@limiter.limit("60/minute")
def admin_create_landlord_document(
    request: Request,
    ...
    try:
        key = build_landlord_object_key(landlord_id, raw_name)
