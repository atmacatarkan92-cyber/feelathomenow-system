...
Tenant isolation policies compare rows to transaction-local GUCs:
  ...
  app.current_email_verification_token_hash — trusted auth-only: SELECT email_verification_tokens by token_hash
    before org context (see apply_pg_email_verification_token_hash_lookup)
...
rls_password_reset_token_hash, rls_email_verification_token_hash, rls_inquiry_id) and:
...
def _rls_after_begin(session: Session, transaction, connection) -> None:
    ...
    evth = session.info.get("rls_email_verification_token_hash")
    if evth is not None:
        connection.execute(
            text("SET LOCAL app.current_email_verification_token_hash = :v"),
            {"v": str(evth)},
        )
...
def apply_pg_email_verification_token_hash_lookup(session: Session, token_hash: str | None) -> None:
    """
    Trusted server-only: allow SELECT/UPDATE email_verification_tokens by token_hash before org GUC is set.
    Pass None to clear.
    """
    if not token_hash or not str(token_hash).strip():
        session.info.pop("rls_email_verification_token_hash", None)
        if session.in_transaction():
            session.execute(text("SET LOCAL app.current_email_verification_token_hash = ''"))
        else:
            session.execute(text("SELECT 1"))
        return
    s = str(token_hash).strip()
    session.info["rls_email_verification_token_hash"] = s
    if session.in_transaction():
        session.execute(
            text("SET LOCAL app.current_email_verification_token_hash = :v"),
            {"v": s},
