...
function LandlordInvoicesPage() {
  ...
  const tenantDisplay = (inv) =>
    (inv.tenant_name && inv.tenant_name.trim()) ? inv.tenant_name : (inv.tenant_email || "—");
  const unitDisplay = (inv) => inv.unit_title || inv.unit_name || inv.unit_id || "—";
  const propertyDisplay = (inv) => inv.property_title || inv.property_id || "—";
            ...
            <tbody>
              {list.map((inv) => (
                <tr key={inv.id} style={{ borderBottom: "1px solid #F1F5F9" }}>
                  ...
                  <td style={{ padding: "12px 8px" }}>{tenantDisplay(inv)}</td>
                  <td style={{ padding: "12px 8px" }}>{propertyDisplay(inv)}</td>
                  <td style={{ padding: "12px 8px" }}>{unitDisplay(inv)}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default LandlordInvoicesPage;
