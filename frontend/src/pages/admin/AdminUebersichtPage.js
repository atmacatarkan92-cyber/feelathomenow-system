  ...
  return (
    <div className="relative overflow-hidden rounded-[14px] border border-[var(--border)] bg-[var(--bg-card)] p-5">
      ...
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="text-[9px] font-bold uppercase tracking-[1px] text-[var(--text-muted)]">{titel}</div>
        <span className="inline-flex items-center whitespace-nowrap rounded-full border border-[var(--border)] bg-white/[0.06] px-2.5 py-0.5 text-[10px] font-bold text-[var(--text-muted)]">
          {badge}
        </span>
      </div>
      ...
      {hinweis ? (
        <div className="mt-3 text-[11px] leading-relaxed text-[var(--text-muted)]">{hinweis}</div>
      ) : null}
    </div>
  );
}
...
function SchnellzugriffKarte({
  ...
  return (
    <div className="flex min-h-[210px] flex-col gap-3.5 rounded-[14px] border border-[var(--border)] bg-[var(--bg-card)] p-5">
      ...
      <div>
        <h3 className="mb-2 text-[18px] font-bold tracking-tight text-[var(--text-primary)]">{titel}</h3>
        <p className="m-0 text-[13px] leading-relaxed text-[var(--text-muted)]">{text}</p>
      </div>
    ...
    <div data-testid="admin-dashboard-page" className="min-h-screen bg-[var(--bg-page)] text-[var(--text-primary)]">
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        <select
          ...
          className="rounded-[8px] border border-[var(--border)] bg-[var(--bg-inner)] px-3 py-2 text-[13px] text-[var(--text-primary)]"
        >
          {(() => {
            const d = new Date();
            const options = [];
      ...
      {kpis && !kpisLoading && (
        <>
          ...
          {Array.isArray(kpis.assumptions) && kpis.assumptions.length > 0 && (
            <div className="rounded-[14px] border border-[var(--border)] bg-[var(--bg-card)] p-6">
              <h3 className="m-0 text-[9px] font-bold uppercase tracking-[1px] text-[var(--text-muted)]">Annahmen & Limitationen</h3>
              <p className="mb-3 mt-2 text-[13px] text-[var(--text-muted)]">So wurden die KPIs berechnet. Geschätzte oder nicht verfügbare Werte sind gekennzeichnet.</p>
              <ul className="m-0 list-disc space-y-1.5 pl-5 text-[13px] text-[var(--text-muted)]">
      ...
      <div className="rounded-[14px] border border-[var(--border)] bg-[var(--bg-card)] p-6">
        <h3 className="mb-4 text-[16px] font-bold text-[var(--text-primary)]">Finanzentwicklung letzte 6 Monate</h3>
        {monthlyChartsLoading ? (
          <p className="text-[13px] text-[var(--text-muted)]">Lade Monatsdaten…</p>
        ) : monthlyChartsError ? (
          <p className="text-[13px] text-[#f87171]">{monthlyChartsError}</p>
        ) : financeChartData.length === 0 ? (
          <p className="text-[13px] text-[var(--text-muted)]">Keine Daten vorhanden</p>
        ) : (
      ...
      <div className="rounded-[14px] border border-[var(--border)] bg-[var(--bg-card)] p-6">
        <h3 className="mb-4 text-[16px] font-bold text-[var(--text-primary)]">Belegung Rooms letzte 6 Monate</h3>
        {monthlyChartsLoading ? (
          <p className="text-[13px] text-[var(--text-muted)]">Lade Monatsdaten…</p>
        ) : monthlyChartsError ? (
          <p className="text-[13px] text-[#f87171]">{monthlyChartsError}</p>
            ...
            <table className="w-full border-collapse text-[13px] text-[var(--text-primary)]">
              <thead className="bg-[var(--bg-inner)]">
                <tr className="text-left">
                  <th className="px-3 py-3 text-[9px] font-bold uppercase tracking-[0.8px] text-[var(--text-muted)]">
                    Rechnungsnummer
                  </th>
                  <th className="px-3 py-3 text-[9px] font-bold uppercase tracking-[0.8px] text-[var(--text-muted)]">
                    Betrag
